...
void gd_printf(const char *format, ...) {
    ...
    char buf[0x100];
    char *csr = buf;
    char spec[8] = { 0 };    // specifier string
    UNUSED u32 pad40;
    union PrintVal val;
    va_list args;
...
void parse_p1_controller(void) {
    ...
    OSContPad *currInputs = NULL;                        // 34
    OSContPad *prevInputs = NULL;                        // 30
    ...
    for (s32 i = 0; ((s32) i) < sDebugViewsCount; i++) {
    ...
    for (u32 i = 0; i < sizeof(OSContPad); i++) {
